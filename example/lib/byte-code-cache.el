...
;; Code taken from Cedet package
(defvar bcc-cedet-dir-sep-char (if (boundp 'directory-sep-char)
...
(defun bcc-cedet-directory-name-to-file-name (referencedir)
...
Converts directory seperation characters into ! characters."
  (let ((file referencedir)
    ...
    (when (memq system-type '(windows-nt ms-dos))
      ...
      (setq dir-sep-string (char-to-string bcc-cedet-dir-sep-char))
      ...
      (if (eq (aref file 1) ?:)
         (setq file (concat dir-sep-string
                            ...
                            (if (eq (aref file 2) bcc-cedet-dir-sep-char)
                                ""
                              dir-sep-string)
                            (substring file 2)))))
...
.elc"
  ...
  (concat
   ...
   (bcc-cedet-directory-name-to-file-name
    (concat
